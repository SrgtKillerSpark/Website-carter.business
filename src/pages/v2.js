const GITHUB_USERNAME = 'SrgtKillerSpark';
let projects = [];
  ...
  try {
    ...
    if (projects.length === 0) {
      projects = [{
        ...
        description: 'Check back later for projects!',
        tags: [],
    ...
    projects = [{
      ...
      description: 'Unable to load projects. Click to view on GitHub.',
      tags: [],
      link: `https://github.com/${GITHUB_USERNAME}`
    }];
    projectsLoaded = true;
...
function updateProjectsUI() {
  ...
  if (slide && dots) {
    ...
    dots.innerHTML = projects.map((_, i) => 
      `<span class="dot ${i === currentProjectIndex ? 'active' : ''}" data-index="${i}"></span>`
    ).join('');
  }
}
    ...
    <div class="v2-main">
      <aside class="v2-sidebar">
        ...
        <nav class="v2-social-links">
          <svg class="constellation-svg" viewBox="0 0 200 280" preserveAspectRatio="xMidYMid meet">
            <!-- Zigzag constellation lines connecting social icons -->
            <path d="M 16 28 L 40 28 L 60 56 L 16 56" stroke="var(--accent)" stroke-width="1.5" fill="none" opacity="0.4"/>
            <path d="M 16 56 L 50 56 L 30 84 L 16 84" stroke="var(--accent)" stroke-width="1.5" fill="none" opacity="0.4"/>
            <path d="M 16 84 L 45 84 L 55 112 L 16 112" stroke="var(--accent)" stroke-width="1.5" fill="none" opacity="0.4"/>
            <path d="M 16 112 L 35 112 L 50 140 L 16 140" stroke="var(--accent)" stroke-width="1.5" fill="none" opacity="0.4"/>
            <path d="M 16 140 L 60 140 L 40 168 L 16 168" stroke="var(--accent)" stroke-width="1.5" fill="none" opacity="0.4"/>
            <!-- Constellation dots at vertices -->
            <circle cx="40" cy="28" r="3" fill="var(--accent)" opacity="0.6"/>
            <circle cx="60" cy="56" r="2" fill="var(--accent)" opacity="0.5"/>
            <circle cx="50" cy="56" r="2" fill="var(--accent)" opacity="0.5"/>
            <circle cx="30" cy="84" r="3" fill="var(--accent)" opacity="0.6"/>
            <circle cx="45" cy="84" r="2" fill="var(--accent)" opacity="0.5"/>
            <circle cx="55" cy="112" r="2" fill="var(--accent)" opacity="0.5"/>
            <circle cx="35" cy="112" r="2" fill="var(--accent)" opacity="0.5"/>
            <circle cx="50" cy="140" r="3" fill="var(--accent)" opacity="0.6"/>
            <circle cx="60" cy="140" r="2" fill="var(--accent)" opacity="0.5"/>
            <circle cx="40" cy="168" r="2" fill="var(--accent)" opacity="0.5"/>
          </svg>
      ...
      <aside class="v2-actions">
        ...
        <a href="https://www.linkedin.com/in/andrew-carter-538a1224a/" target="_blank" rel="noopener noreferrer" class="v2-action-link">
          <div class="action-icon-wrapper">
            <svg viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
          </div>
          <span>LinkedIn</span>
        </a>
        <a href="https://github.com/SrgtKillerSpark" target="_blank" rel="noopener noreferrer" class="v2-action-link">
          <div class="action-icon-wrapper">
...
function renderProject(project) {
  if (!project) return '<p class="loading-text">Loading...</p>';
  ...
  return `
    ...
    ${project.link ? `<a href="${project.link}" target="_blank" rel="noopener noreferrer" class="project-link">View on GitHub →</a>` : ''}
  `;
}
